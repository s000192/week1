...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            component puzzleRangeProof = RangeProof(252);
            puzzleRangeProof.in <== puzzle[i][j];
            puzzleRangeProof.range[0] <== 1;
            puzzleRangeProof.range[1] <== 9;
            assert(puzzleRangeProof.out);

            component solutionRangeProof = RangeProof(252);
            solutionRangeProof.in <== solution[i][j];
            solutionRangeProof.range[0] <== 1;
